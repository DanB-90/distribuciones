...
for label in param_labels:
    valor = st.number_input(f"Ingresar {label}:", value=0.0, step=0.0001, format="%.4f")
...
if modo == "Probabilidad P(X ≤ x)":
    x = st.number_input("Ingresa el valor de x:", value=0.0, step=0.0001, format="%.4f")
    p = None
else:
    p = st.number_input("Ingresa la probabilidad p (entre 0 y 1):", min_value=0.0, max_value=1.0, value=0.0500, step=0.0001, format="%.4f")
    x = None
